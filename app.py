...
def gen_wind_speed(old_figure):
    ...
    layout = Layout(
        ...
        yaxis=dict(
            range=[min(0, min(wind_val)), max(45, max(wind_val)+max(wind_error))],
            showline=False,
            zeroline=False,
            nticks=max(6, round(prev_val/10))
        ),
        margin=Margin(
            t=45,
            l=50,
